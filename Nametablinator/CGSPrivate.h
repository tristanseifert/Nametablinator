...
extern "C" {
	...
	typedef int CGSTransitionHandle;			// DSJ
...
#pragma mark Transitions
	  ...
	  typedef struct {
		    ...
		    float *backColor;		/* Null for black otherwise pointer to 3 float array with RGB value */  // DSJ: Let's spell it the American way! :-)
	  } CGSTransitionSpec;
	  
	  extern CGError CGSNewTransition(const CGSConnection cid, const CGSTransitionSpec* spec, CGSTransitionHandle * transitionHandle);		// DSJ
	  extern CGError CGSInvokeTransition(const CGSConnection cid, CGSTransitionHandle transitionHandle, float duration);									// DSJ
	  extern CGError CGSReleaseTransition(const CGSConnection cid, CGSTransitionHandle transitionHandle);																	// DSJ
